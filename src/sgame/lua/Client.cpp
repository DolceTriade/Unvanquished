...
// @within Client
GET_FUNC( connected,
          lua_pushboolean( L, c->ent->client->pers.connected == clientConnected_t::CON_CONNECTED ) )
...
GET_FUNC( evos,
          lua_pushnumber( L,
                          c->ent->client->pers.credit / static_cast<double>( CREDITS_PER_EVO ) ) )
/// The client's weapon.
...
int Methodteleport( lua_State* L, Client* c )
{
	...
	G_TeleportPlayer( c->ent, VEC2GLM( origin ), VEC2GLM( angles ), 0 );
	return 0;
}
...
namespace Shared {
namespace Lua {
LUACORETYPEDEFINE( ::Lua::Client )

template <>
void ExtraInit<::Lua::Client>( lua_State* /*L*/, int /*metatable_index*/ )
{}
}  // namespace Lua
}  // namespace Shared
